...
var TEMPLATE_EXPORTS = {filter: filter, utils: utils};
...
function Template(str, options) {
  ...
  if(!options.cache) {
    if(options && options.settings) {
      options.cache = options.settings.cache !== 'development';
    } else {
      options.cache = false;
    }
  }
  ...
  if(options.cache && !(this._getCache() instanceof options.cacheHandler)) {
    ...
    options.cacheContext[options._cacheProp] = construct.apply(this,
                                                               cacheOptions);
  }
}
